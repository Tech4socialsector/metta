...
class StockSaleReport {
	...
	inject_styles() {
		...
		$(`<style id="ssr-styles">
			...
			.ssr-page .ssr-card.returns .ssr-card-label { color: #a3341f; }
			...
			.ssr-page .ssr-card.returns .ssr-card-value { color: #a3341f; }
	...
	make_section_title() {
		$(`
			<div class="ssr-section-title">SALES</div>
	...
	render(rows) {
		...
		const gross_value = rows.reduce((sum, r) => sum + flt(r.gross_value), 0);
		const return_value = rows.reduce((sum, r) => sum + flt(r.return_value), 0);
		const net_value = rows.reduce((sum, r) => sum + flt(r.net_value), 0);

		this.cards_area.html(`
			<div class="ssr-card">
				<div class="ssr-card-label">${__("Items Sold")}</div>
				<div class="ssr-card-value">${rows.length}</div>
			</div>
			<div class="ssr-card">
				<div class="ssr-card-label">${__("Gross Sales Value")}</div>
				<div class="ssr-card-value">${format_currency(gross_value)}</div>
			</div>
			<div class="ssr-card returns">
				<div class="ssr-card-label">${__("Returns Value")}</div>
				<div class="ssr-card-value">${format_currency(return_value)}</div>
			</div>
			<div class="ssr-card">
				<div class="ssr-card-label">${__("Net Sales Value")}</div>
				<div class="ssr-card-value">${format_currency(net_value)}</div>
			</div>
		`);

		if (!rows.length) {
			this.table_area.html(`<p class="text-muted">${__("No sales found for this range.")}</p>`);
			return;
		}
		...
		const body = rows
			.map(
				...
				<tr>
					...
					<td class="text-center">${flt(row.net_qty)}</td>
					<td class="text-center">${format_currency(row.gross_value)}</td>
					<td class="text-center">${format_currency(row.return_value)}</td>
					<td class="text-center">${format_currency(row.net_value)}</td>
		...
		const total_row = `
			<tr class="ssr-total">
				...
				<td class="text-center">${rows.reduce((s, r) => s + flt(r.qty_returned), 0)}</td>
				<td class="text-center">${rows.reduce((s, r) => s + flt(r.net_qty), 0)}</td>
				<td class="text-center">${format_currency(gross_value)}</td>
				<td class="text-center">${format_currency(return_value)}</td>
				<td class="text-center">${format_currency(net_value)}</td>
	...
	get_export_data() {
		...
		const data = rows.map((row) => [
			...
			flt(row.net_qty),
			format_currency(row.gross_value),
			format_currency(row.return_value),
			format_currency(row.net_value),
			format_currency(row.gst_collected),
		]);

		const total_qty_sold = rows.reduce((s, r) => s + flt(r.qty_sold), 0);
		const total_qty_returned = rows.reduce((s, r) => s + flt(r.qty_returned), 0);
		const total_net_qty = rows.reduce((s, r) => s + flt(r.net_qty), 0);
		const total_gross = rows.reduce((s, r) => s + flt(r.gross_value), 0);
		const total_return = rows.reduce((s, r) => s + flt(r.return_value), 0);
		const total_net = rows.reduce((s, r) => s + flt(r.net_value), 0);
